...
SkillSync is a **Git-inspired CLI tool** for managing, versioning, and collaborating on [OpenClaw](https://openclaw.ai) Skill files. The command is `ski`.
...
SkillSync 是一个**类 Git 的命令行工具**，专为管理、版本化和团队协作 [OpenClaw](https://openclaw.ai) Skill 文件而设计。命令为 `ski`。

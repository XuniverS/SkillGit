...
async function cmdConfig(action, key, value, options) {
  ...
  // 解析 `skillsync config user.name "Alice"` 格式
  if (action && key && !value) {
  ...
  logger.error('Usage: skillsync config [list|get <key>|set <key> <value>]');
}
...
async function cmdRemote(action, name, url) {
  ...
  logger.error('Usage: skillsync remote [set-url <name> <url> | get-url <name> | list]');
}

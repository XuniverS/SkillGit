...
/**
 * skl merge --ai         使用 AI 合并所有冲突文件
 * skl merge --show-prompt 只显示 AI prompt 不实际调用
 * skl merge <skill>      合并指定 skill 的冲突
 */
async function cmdMerge(skillArg, options) {
  ...
  if (!options.ai && !options.yes) {
    ...
    if (!answer.proceed) {
      logger.info('Merge cancelled. Fix conflicts manually, then `skl add` and `skl commit`.');
      return;
    }
  }
  ...
  if (!apiKey) {
    logger.error(
      'No API key configured.\n  Run: skl config set user.openapiKey <your-key>\n  Or set env: OPENCLAW_API_KEY=<your-key>'
    );
    process.exit(1);
  }
  ...
  if (manualNeeded > 0) {
    ...
    logger.plain('  skl add .');
    logger.plain('  skl commit -m "resolve merge conflicts"');
  }
}

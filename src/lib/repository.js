...
class Repository {
  ...
  static requireRepo(cwd) {
    ...
    if (!root) {
      throw new Error(
        'Not a SkillGit repository (or any of the parent directories). Run `skl init` to initialize.'
      );
    }
  ...
  async getConfig() {
    if (!fs.existsSync(this.configPath)) {
      throw new Error('Repository config not found. Run `skl init` first.');
    }
  ...
  async commit(message, options = {}) {
    ...
    if (Object.keys(index.staged).length === 0) {
      throw new Error('Nothing to commit. Use `skl add <skill>` to stage changes.');
    }
  ...
  async getDiff(skillName) {
    ...
    if (!tracked) {
      throw new Error(`Skill '${skillName}' is not tracked. Run \`skl add ${skillName}\` first.`);
    }
